...
def plotSpeedup( xranks, baseline_times, stealing_r0, stealing_r1, stealing_r2, nostealing_r1, nostealing_r2, output_file_path, text_header):
...
if __name__ == "__main__":
 ...
 for t in threads:
     ...
     times_rep1_no_stealing =[]
     times_rep2_no_stealing =[]
     for r in ranks:
       times_rep0_no_stealing.append( st.mean([x.samoa_phase_time for x in p_list[0] if x.n_ranks==r]) )
       times_rep1_no_stealing.append( st.mean([x.samoa_phase_time for x in p_list[1] if x.n_ranks==r]) )
       times_rep2_no_stealing.append( st.mean([x.samoa_phase_time for x in p_list[2] if x.n_ranks==r]) )
     ...
     for r in ranks:
       ...
       times_rep1_stealing.append( st.mean([x.samoa_phase_time for x in p_list[1] if x.n_ranks==r]) )
       times_rep2_stealing.append( st.mean([x.samoa_phase_time for x in p_list[2] if x.n_ranks==r]) )
     plotSpeedup(ranks, times_rep0_no_stealing, times_rep0_stealing, times_rep1_stealing, times_rep2_stealing, times_rep1_no_stealing, times_rep2_no_stealing, target_folder_plot+"/t"+str(t), "threads="+str(t)) 
