...
VARIATION_DIR=${LOG_DIR}"/TopologyNearest"
mkdir ${VARIATION_DIR}
for RUN in {1..${N_RUNS}}
do
...
export SOME_INDEX=$(($SOME_INDEX+1))    # for plotting
VARIATION_DIR=${LOG_DIR}"/TopologyDistant"
mkdir ${VARIATION_DIR}
for RUN in {1..${N_RUNS}}
do
    eval "run_experiment" >>& ${VARIATION_DIR}/R${RUN}.log
...
module load ${CHAMELEON_VERSION}
export TOPO_MIGRATION_STRAT=0 # topology aware most distant

export SOME_INDEX=$(($SOME_INDEX+1))    # for plotting
VARIATION_DIR=${LOG_DIR}"/ChameleonNoTopo"
mkdir ${VARIATION_DIR}
for RUN in {1..${N_RUNS}}
do
    eval "run_experiment" >>& ${VARIATION_DIR}/R${RUN}.log
...
export SOME_INDEX=$(($SOME_INDEX+1))    # for plotting
VARIATION_DIR=${LOG_DIR}"/NoCommThread"
mkdir ${VARIATION_DIR}
for RUN in {1..${N_RUNS}}
do
    eval "run_experiment" >>& ${VARIATION_DIR}/R${RUN}.log
