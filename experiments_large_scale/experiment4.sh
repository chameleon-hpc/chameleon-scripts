...
DIR_RESULT="results_experiment4"
...
do
    CUR_VAL=$((i_node*NUM_TASKS/N_NODES))
    MXM_PARAMS="${MXM_PARAMS}${CUR_VAL} "
done

for gran in "${TASK_GRANULARITY[@]}"
do
    echo "===== Running experiment 4 for ${current_name}, granularity=${gran}, n_threads=${tmp_n_threads}"
