...
def parseSamoaLog(filename):
	...
	lbfreq_pattern = re.compile(".* frequency: ([0-9]+)")
        ...
        #     (r0,t0)  Cell update throughput solver:      139.5494 M/s
	...
	for line in file:
	  ...
	  if m:
	    lbfreq = int(m.group(1))
          m=re.match(cell_throughput_pattern)
          if m:
            cell_throughput = int(m.group(1))
	...
	print (chameleon,"\t",ranks,"\t",threads,"\t",lbfreq,"\t",sections,"\t",dmin,"\t",dmax,"\t",time, "\t", cell_throughput)
