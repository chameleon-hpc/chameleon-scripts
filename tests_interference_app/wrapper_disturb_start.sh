...
#RANKS_TO_DISTURB=1,2
export DEBUG=1

args="--type=${DIST_TYPE} --rank_number=${PMI_RANK} --window_us_comp=${DIST_WINDOW_US_COMP} --window_us_pause=${DIST_WINDOW_US_PAUSE} --use_multiple_cores=${DIST_NUM_THREADS} --use_random=${DIST_RANDOM} --window_us_size_min=${DIST_WINDOW_US_COMP_MIN} --window_us_size_max=${DIST_WINDOW_US_COMP_MAX} --use_ram=${DIST_RAM_MB} --com_type=${DIST_COM_MODE} --com_size=${DIST_COM_SIZE} --ranks_to_disturb=${RANKS_TO_DISTURB}"
