...
export DIST_NUM_THREADS=${SB_OMP_NUM_THREADS:-23}
...
export DIST_WINDOW_US_COMP=200000
export DIST_WINDOW_US_PAUSE=100000
...
do
    ...
    if [[ "1" = "${DISTURB_RANKS}" ]]; then
        (${MPIEXEC} ${FLAGS_MPI_BATCH} --mem=1G --export=DIST_PID_FOLDER,FILENAME,NAME,OMP_PLACES,OMP_PROC_BIND,OMP_NUM_THREADS,DIST_NUM_THREADS,DIST_TYPE,DIST_WINDOW_US_COMP,DIST_WINDOW_US_PAUSE,DIST_WINDOW_US_COMP_MIN,DIST_WINDOW_US_COMP_MAX,DIST_RANDOM,DIST_RAM_MB,DIST_COM_MODE,DIST_COM_SIZE --oversubscribe -- ./wrapper_disturb_start.sh 2>&1 >> ${FILENAME}.txt &)
    fi
    ${MPIEXEC} ${FLAGS_MPI_BATCH} --mem=120G --export=ITERATION_NUM,OMP_PLACES,OMP_PROC_BIND,OMP_NUM_THREADS,PROG,NAME,DIST_NUM_THREADS,DIST_TYPE,DIST_WINDOW_US_COMP,DIST_WINDOW_US_PAUSE,DIST_WINDOW_US_COMP_MIN,DIST_WINDOW_US_COMP_MAX,DIST_RANDOM,DIST_RAM_MB,FILENAME --oversubscribe -- ./wrapper_application.sh 2>&1 >> ${FILENAME}.txt
    if [[ "1" = "${DISTURB_RANKS}" ]]; then
        ${MPIEXEC} ${FLAGS_MPI_BATCH} --mem=100M --export=DIST_PID_FOLDER,FILENAME,NAME,OMP_PLACES,OMP_PROC_BIND,OMP_NUM_THREADS,DIST_NUM_THREADS,DIST_TYPE,DIST_WINDOW_US_COMP,DIST_WINDOW_US_PAUSE,DIST_WINDOW_US_COMP_MIN,DIST_WINDOW_US_COMP_MAX,DIST_RANDOM,DIST_RAM_MB,DIST_COM_MODE,DIST_COM_SIZE --oversubscribe -- ./wrapper_disturb_cleanup.sh 2>&1 >> ${FILENAME}.txt
    fi
done
